...
function openImageViewer(index) {
  ...
  // Show loading animation BEFORE setting the src
  showLoading(viewerImg);

  // Set up load and error handlers
  viewerImg.onload = () => {
    hideLoading(); // Hide loader on successful load
...
document.addEventListener('fullscreenchange', () => {
  if (document.fullscreenElement === viewer) {
    ...
    // You might want to hide the loader if it's somehow still visible when entering fullscreen
    // although ideally it should be gone after image load.
    // hideLoading();
  } else {
...
function applyTransform() {
  if (scale > 1 && viewer.classList.contains('fullscreen')) {
    requestAnimationFrame(() => {
        // Use offsetWidth/Height multiplied by scale for boundary calc
        const viewerWidth = viewer.clientWidth;
        const viewerHeight = viewer.clientHeight;
        const scaledWidth = viewerImg.offsetWidth * scale;
        const scaledHeight = viewerImg.offsetHeight * scale;

        // Ensure boundaries are calculated correctly
        // If scaled image is smaller than viewer, minX/Y should be 0
        const minX = Math.min(0, viewerWidth - scaledWidth);
        const maxX = Math.max(0, viewerWidth - scaledWidth); // Should likely be 0 if scaledWidth > viewerWidth
        const minY = Math.min(0, viewerHeight - scaledHeight);
        const maxY = Math.max(0, viewerHeight - scaledHeight); // Should likely be 0 if scaledHeight > viewerHeight
...
viewerImg.addEventListener('mousedown', (e) => {
  // Allow mousedown only if image is visible (loaded) and conditions met
...
document.addEventListener('mouseup', (e) => {
  ...
  // Navigation click logic (only if not dragging and not fullscreen)
  if (!wasDragging && !viewer.classList.contains('fullscreen') && viewerImg.style.visibility !== 'hidden') { // Add check for visibility
      const rect = viewerImg.getBoundingClientRect();
...
viewerImg.addEventListener('touchstart', (e) => {
  // Allow touchstart only if image is visible
  ...
  if (touches.length === 1) {
    ...
    if (viewer.classList.contains('fullscreen') && scale > 1) {
        e.preventDefault(); // Prevent scroll only if panning is possible
...
viewerImg.addEventListener('touchmove', (e) => {
  // Only handle if touch started on the element and image is visible
...
viewerImg.addEventListener('touchend', (e) => {
  ...
  if (isPinching) {
    ...
    applyTransform(); // Final boundary check
  ...
  if (touches.length === 0) {
      resetTouchState(); // Reset only if all touches are up and no navigation happened
  }
